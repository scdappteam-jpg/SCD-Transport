...
วิธีง่ายสุด: เปิดไฟล์ `Start-SmartLogistics.cmd` (จะเปิดทั้ง Node server และ Barcode Scanner service ให้อัตโนมัติ)
...
## ระบบสแกนบาร์โค้ด (python-scanqrcode-tps)

ฝั่งพนักงาน (หน้า `/mobile`) สแกน House ได้ 2 แบบ:

1. **เครื่องยิงบาร์โค้ด / พิมพ์เอง** — ใส่เลขในช่อง House โดยตรง
2. **ถ่ายรูปสแกนด้วยกล้อง** — กดปุ่ม "สแกนด้วยกล้อง / Camera" แล้วถ่ายรูปป้ายสินค้า รูปจะถูกส่งไปให้ Python Scanner (pyzbar + EasyOCR หมุนภาพ 4 มุม) ถอดรหัสแล้วเติม House ให้อัตโนมัติ — ใช้ `capture="environment"` เปิดกล้องหลังได้บน HTTP ธรรมดา ไม่ต้องใช้ HTTPS หรือ live video stream

### ติดตั้งครั้งแรก (ครั้งเดียว)

```powershell
cd python-scanqrcode-tps
python -m venv venv
venv\Scripts\activate
pip install -r requirement.txt
```

### วิธีรัน Scanner service
...
- รันที่ `http://localhost:5000` (ใช้ `SCAN_PORT` เพื่อเปลี่ยน port)
- Swagger UI: `http://localhost:5000/docs`
- Node server ทำ proxy ให้อัตโนมัติที่ `POST /api/scan-barcode` → ตั้งค่า `SCAN_SERVICE_URL` เพื่อชี้ไปที่อื่น (default `http://localhost:5000/scan`)
- ต้องการให้มือถือเรียกตรงถึง scanner service โดยไม่ผ่าน Node: ตั้ง `window.SMART_LOGISTICS_SCAN_API` ใน `public/config.js`

> หมายเหตุ: ถ้าไม่ได้เปิด Scanner service ปุ่มสแกนกล้องจะแจ้ง error และยังใช้เครื่องยิงบาร์โค้ดหรือพิมพ์ House ได้ตามปกติ

> หมายเหตุ: `Start-SmartLogistics.cmd` เปิดทั้ง Node server และ Scanner service ให้อัตโนมัติ — หรือรันเอง 2 terminal (Python ก่อนที่ port 5000 แล้วค่อย `node server.js` ที่ port 3000)
